...
import { SceneItem, BackgroundImage, AspectRatio, StoredScene, SceneFilter, BubbleData } from './types';
...
const App: React.FC = () => {
  ...
  const handleAddItem = (type: 'character' | 'sticker', payload?: string) => {
  ...
  const handleSaveBubble = (imgSrc: string, data: BubbleData) => {
      if (editingBubbleId) {
          // Editing existing
          setItems(prev => prev.map(item => item.id === editingBubbleId ? { ...item, src: imgSrc, bubbleData: data } : item));
  ...
  const handleEditBubbleTrigger = (id: string) => {
  ...
  const handleMoveLayer = (index: number, direction: 'up' | 'down') => {
      setItems(prev => {
          ...
          // Re-assign Z-Indexes to match array order (clean up)
  ...
  const handleCaptureScene = async () => {
    setSelectedId(null);
    setIsCapturing(true);
    // Close panel for capture
    const wasLayerPanelOpen = showLayers;
    if(wasLayerPanelOpen) setShowLayers(false);
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            if(wasLayerPanelOpen) setShowLayers(true);
        }
    }, 200); 
  };
    ...
    <div className="flex h-[100dvh] bg-[#FFF0F5] font-sans overflow-hidden relative select-none text-gray-700">
      ...
      <div className="flex-1 flex flex-col relative w-full h-full transition-all duration-300">
          ...
          <header className={`absolute top-0 w-full z-40 p-4 flex justify-between items-start transition-transform duration-300 ${uiVisible && !isCapturing ? 'translate-y-0' : '-translate-y-32'}`}>
            <div className="flex gap-2 items-start">
                <div className="bg-white/80 backdrop-blur-md px-4 py-2 rounded-full shadow-lg border-2 border-pink-200 flex items-center gap-2 animate-slide-up">
                    ...
                    <h1 className="text-lg font-bold text-[#FF8FAB] hidden sm:block">Delani Studio</h1>
                </div>
                
                {/* Layer Toggle */}
                <button 
                    onClick={() => setShowLayers(!showLayers)} 
                    className={`p-2 rounded-full shadow-lg border-2 border-white transition-colors animate-slide-up ${showLayers ? 'bg-pink-400 text-white' : 'bg-white text-gray-400'}`}
                >
                    <Layers size={20}/>
                </button>
            </div>
          ...
          <main className="flex-1 w-full h-full relative flex">
            ...
            {/* Layer Panel (Sliding in) */}
            <div className={`h-full z-30 transition-all duration-300 absolute left-0 top-0 bottom-0 ${showLayers ? 'translate-x-0 w-64' : '-translate-x-full w-0'}`}>
                <LayerPanel 
                    items={items} 
            ...
            <div className="flex-1 h-full relative" onClick={() => { if(showLayers) setShowLayers(false); }}>
                <Stage 
                    key={resetKey}
                    items={items}
                    ...
                    onEditBubble={handleEditBubbleTrigger}
                />
            </div>
          ...
          <footer className={`absolute bottom-6 w-full z-40 transition-transform duration-300 flex justify-center pointer-events-none ${uiVisible && !isCapturing ? 'translate-y-0' : 'translate-y-[150%]'}`}>
             <div className="pointer-events-auto flex flex-col items-center gap-4 w-full max-w-lg px-4">
                ...
                <div className="bg-white/95 backdrop-blur-xl shadow-2xl rounded-full p-2 px-6 border-2 border-white ring-4 ring-pink-100 flex items-center justify-between gap-2 w-full">
                    ...
                    <ToolBtn icon={<MessageCircleHeart size={22}/>} label="Fala" color="blue" onClick={() => { setEditingBubbleId(null); setShowBubbleModal(true); }}/>
      ...
      {showBubbleModal && (
        <BubbleModal 
            onClose={() => { setShowBubbleModal(false); setEditingBubbleId(null); }} 
            onSave={handleSaveBubble}
            initialData={editingBubbleId ? items.find(i => i.id === editingBubbleId)?.bubbleData : undefined}
        />
      )}
